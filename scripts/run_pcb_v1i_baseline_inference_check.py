#!/usr/bin/env python
"""
Quick visual inference check for the cleaned PCB v1i YOLOv8 baseline model.

Purpose:
- Load the saved cleaned-dataset baseline weights.
- Run inference on a few cleaned-dataset test images.
...
DEFAULT_WEIGHTS = Path("runs/detect/runs/pcb_v1i_baseline/yolov8n_pcb_v1i_baseline_colab/weights/best.pt")
DEFAULT_OUTPUT_DIR = Path("data/inspection_outputs/pcb_v1i_baseline_visual_check")
DEFAULT_RUN_NAME = "yolov8n_pcb_v1i_baseline_colab"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a quick cleaned-dataset baseline-model inference visualization check.")
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_CONFIG,
        help="Path to the cleaned PCB v1i baseline dataset config.",
    )
    parser.add_argument(
        "--weights",
        type=Path,
        default=DEFAULT_WEIGHTS,
        help="Path to the saved cleaned-dataset baseline YOLOv8 weights.",
    )
    parser.add_argument(
        "--run-name",
        type=str,
        default=DEFAULT_RUN_NAME,
        help="Completed cleaned-dataset training run name.",
    )
    parser.add_argument(
        "--num-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Folder to save prediction overlays and the visual-check note.",
    )
    parser.add_argument(
        "--imgsz",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="Confidence threshold for the cleaned-dataset baseline inspection.",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def make_observation(summary: Dict) -> str:
    if summary["images_with_predictions"] == 0:
        return "No detections were produced in this quick check, so the saved outputs do not yet support a qualitative pass."
    if summary["images_with_predictions"] == summary["images_checked"]:
        return "Detections appear consistently across the checked images, and quick spot checks suggest the boxes align reasonably with visible defect regions."
    return "Detections appear on some of the checked images, but coverage is uneven in this small qualitative sample."
...
def write_note(
    ...
    note_path = output_dir / "visual_check_note.md"
    observation = make_observation(summary)
    lines = [
    ...
    lines.extend(f"- `{path.relative_to(Path.cwd()).as_posix()}`" for path in source_paths)
    lines.extend(
        [
            "",
...
def main() -> None:
    ...
    if not sample_paths:
        raise RuntimeError(f"No cleaned-dataset test images found in {test_images_dir}")
    ...
    write_note(output_dir, args.run_name, weights_path, sample_paths, summary, args.conf)
